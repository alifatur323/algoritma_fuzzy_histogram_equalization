	...
	foreach ($valuex as $y => $valuey) {
		foreach ($valuey as $warna => $nilai) {
			...
			foreach ($dt_him as $key_h => $value_h) {
				...
				if ($min==0) {
					if ($nilai <= $ten) {
						$fuzzifikasi[$x][$y][$idk][$nh] = 1;
					} elseif ($nilai > $min AND $nilai <= $mak) {
						$fuzzifikasi[$x][$y][$idk][$nh] = ($mak - $nilai) / ($mak - $ten);
					} else {
						$fuzzifikasi[$x][$y][$idk][$nh] = 0;
					}
				}elseif ($min!==0 AND $mak!==255) {
					if ($nilai <= $min OR $nilai > $mak) {
						$fuzzifikasi[$x][$y][$idk][$nh] = 0;
					} elseif ($nilai > $min AND $nilai <= $ten) {
						$fuzzifikasi[$x][$y][$idk][$nh] = ($nilai - $min) / ($ten - $min);
					} else {
						$fuzzifikasi[$x][$y][$idk][$nh] = ($mak - $nilai) / ($mak - $ten);
					}
				}else {
...
foreach ($fuzzifikasi as $x => $valuex) {
	foreach ($valuex as $y => $valuey) {
		foreach ($valuey as $idk => $valuek) {
			foreach ($valuek as $nh => $valueh) {
				$nilai_aturan = $detail_aturan[$nh]['naturan'];
				$perkalian[$x][$y][$idk][$nh] = $valueh * $nilai_aturan;
			}
		}
	}
}

foreach ($perkalian as $x => $valuex) {
	foreach ($valuex as $y => $valuey) {
		foreach ($valuey as $idk => $valuek) {
			$defuzzifikasi[$x][$y][$idk] = array_sum($valuek) / array_sum($fuzzifikasi[$x][$y][$idk]);
		}
	}
}
